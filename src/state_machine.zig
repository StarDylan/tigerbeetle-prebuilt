...
pub const tree_ids = struct {
    ...
    pub const AccountEvents = .{
        ...
        // TODO: Those indexes need to be populated during version upgrade.
        .expired_code = 34,
...
pub fn StateMachineType(comptime Storage: type) type {
    ...
    return struct {
        ...
        fn prefetch_query_two_phase_transfers_pending_transfer_callback(
            ...
            transfers.prefetch(
                prefetch_query_two_phase_transfers_pending_transfer_outcome_callback,
                self.prefetch_context.get(.transfers),
            );
        }

        fn prefetch_query_two_phase_transfers_pending_transfer_outcome_callback(
            completion: *TransfersGroove.PrefetchContext,
        ) void {
            const self: *StateMachine = PrefetchContext.parent(.transfers, completion);
            assert(self.prefetch_input != null);
